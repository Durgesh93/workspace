...
class ExperimentMixin:
    ...
    def experiment_app_apply_tag(self) -> None:
        ...
        def _logical_exclusive_commits(git: Git, parent_branch: str, branch: str, cwd) -> list[str]:
            ...
            parent_idx = 0
            last_matched_branch_idx = -1

            for branch_idx, bsig in enumerate(branch_sigs):
                search_idx = parent_idx
                found_idx = -1

                while search_idx < len(parent_sigs):
                    if parent_sigs[search_idx] == bsig:
                        found_idx = search_idx
                        break
                    search_idx += 1

                if found_idx != -1:
                    last_matched_branch_idx = branch_idx
                    parent_idx = found_idx + 1

            return branch_revs[last_matched_branch_idx + 1:]
