    ...
    def experiment_app_apply_tag(self) -> None:
        ...
        with DB(self._db()) as db:
            ...
            for row in rows:
                ...
                if tag and dep:                   
                    parent = db.execute(
                        "SELECT repo, branch,tag,dep FROM experiments where id = ?",(dep,)
                        )[0]
                    parent_branch = parent['branch']

                    exclusive_commits = git.git(
                        "log",
                    ...
                    if exclusive_commits:
                        start_commit = exclusive_commits[-1]
                        exclusive_start[(repo,branch)] = (start_commit,len(exclusive_commits),tag) 


            for (repo,branch), (start_commit,num,tag) in exclusive_start.items():
                repo_path = project_base / repo
                branch_path = repo_path / branch
                git = Git(repo_path=repo_path)
                ...
                rebase_cmd = (
                    ...
                    'clean=$(printf "%s" "$msg" | sed -E \'s/^(\\[[^]]+\\] ?)+//\'); '
                    f'git commit --amend --allow-empty -m "[{tag}] $clean"'
                )
                ...
                log_message('workspace',f'git: Applied commit tags to {num} commits for repo {repo} branch {branch}')       
